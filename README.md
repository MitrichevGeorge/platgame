...
3. Управление:
   `pygame.key.get_pressed()` возращает словарь состояния клавиш, где ключи - клавиши, а значения(булевые) - нажата ли эта клавиша. Например, если мы полученное из функции значение положим в переменную `keys`, то `keys[pygame.K_UP]` будет `True` если клавиша **стрелочка вверх** нажата.
...
while True:
    ...
    keys = pygame.key.get_pressed() #словарь состояний клавиш
